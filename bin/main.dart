...
main(List<String> arguments) {
  ...
  if (host == '' || !host.contains(new RegExp(r'^(http|https)://*.*\b')) || host.endsWith('/')){
    throw new Exception('Host is not specified or has wrong format: [$host]. Format \"http://www.domain.something\". No ending forward slash (\"/\").');
  }
  else {
    host_pass = true;
  }
  ...
  if (input_file == '' || !input_file.contains(new RegExp(r'.*/*.(csv)'))){
    throw new Exception('Source file name is not specified or not a CSV file: [$input_file].');
  }
  else {
    input_pass = true;
  }
  ...
  if (output_file == '' || !output_file.contains(new RegExp(r'.*/*.(csv)'))
    ...
    throw new Exception('Output is neither a CSV file name or a dirctory, or is not specified all: [$output_file].');
  }
  else {
    output_pass = true;
  }
...
void help(){
  ...
  "\t --help  - this help\n"
  "\n"
  "Full comman line call would look like: \$ dart bin/uri_checker/bin/main.dart -h=http://domain.com -i=path/to/csv/file.csv -o=path/to/new/csv_file.csv \n"
  "###############");
  exit(2);
}